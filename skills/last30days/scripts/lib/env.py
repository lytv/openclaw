...
def check_bird_available() -> bool:
    """Check if bird CLI is installed and available in PATH."""
...
def get_available_sources(config: Dict[str, Any]) -> str:
    ...
    has_bird = check_bird_available()

    if has_openai and (has_xai or has_bird):
        return 'both'
    elif has_openai:
        return 'reddit'
    elif has_xai or has_bird:
        return 'x'
    else:
...
def get_missing_keys(config: Dict[str, Any]) -> str:
    ...
    if has_openai and (has_xai or has_bird):
        ...
        return 'x'  # Missing xAI key (and bird)
    elif has_xai or has_bird:
...
def validate_sources(requested: str, available: str, include_web: bool = False) -> tuple[str, Optional[str]]:
    ...
    if requested == 'both':
        if available not in ('both',):
            ...
            return 'none', f"Requested both sources but {missing} key/tool is missing. Use --sources=auto to use available keys."
        if include_web:
            return 'all', None
        return 'both', None
    ...
    if requested == 'reddit':
        if available == 'x':
            return 'none', "Requested Reddit but only xAI key (or bird) is available."
        if include_web:
            return 'reddit-web', None
        return 'reddit', None
