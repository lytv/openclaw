...
def main():
    ...
    parser.add_argument(
        "--use-bird",
        action="store_true",
        help="Force use of bird CLI for X search instead of xAI API",
    )
    ...
    # Only show if not using bird (which bypasses X key need)
    if missing_keys != 'none' and not (missing_keys == 'x' and (env.check_bird_available() or args.use_bird)):
