...
`~/.openclaw/mcp-server-youtube-transcript`
...
```bash
cd ~/.openclaw/mcp-server-youtube-transcript && node --input-type=module -e "
...
```
~/.openclaw/transcripts/YYYY-MM-DD_VIDEO_ID.txt
```
...
message --action send --channel telegram --target CHAT_ID \
  --filePath ~/.openclaw/transcripts/YYYY-MM-DD_VIDEO_ID.txt \
